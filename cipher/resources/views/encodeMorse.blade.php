...
<form id="encode" action="/encode" method="post">
    ...
    <textarea class="area1" name="text1" id="text1" rows="10" cols="50">{{ session('text') ?? ''}}</textarea>&nbsp &nbsp
    &nbsp
    <textarea class="area2" name="text2" id="text2" rows="10"
              cols="50">{{session('encodedText') ?? ''}}</textarea><br><br>


    {{-- FETCH API--}}
<script>
    function fetchApi() {
        let tweet = document.querySelector("#text1").textContent;
    ...
    <script> submitForms = function () {
            ...
            document.getElementById("decodeMorse").submit();
    ...
    <br><br><br><input class="encode" type="submit" value="Submit" onclick="fetchApi() + submitForms()">

    {{-- SAVE AS TEXT FILE  --}}

    <br>
    <button class="button_save" onclick=saveTextAsFile(text2.value,'download.txt')>Download</button>
    <br><br>
</form>
</body>
</html>
